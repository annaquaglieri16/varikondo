...
It was developed by the need to **re-organise**, **standardise** and **combine** variant calling outputs from several callers. It has specific functions which help with:

1. Variant import in R and parse into a data frame with standardised fields the output from several variant callers (see [how fields are standardaised](https://annaquaglieri16.github.io/varikondo/articles/standardise-variant-fields.html)). The function which achieve this are:

  * `import_goi_superfreq()`: to import variants found on genes of interest called by [superFreq](https://github.com/ChristofferFlensburg/superFreq), a caller that allows clonal tracking of cancer sequencing data. Figure \@{fig:superfreq-diagram} sketches how the function works. 
  
  
```{r superfreq-diagram,eval=TRUE,echo=FALSE,fig.cap="Overview of the functionality to import and organise superFreq outputs.",fig.asp=0.7}
...
2. Combine variant calls from different samples within the same patient. 
